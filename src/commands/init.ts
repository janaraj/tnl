...
behaviors:
  - When a request admits more than one reasonable interpretation, the agent MUST ask clarifying questions before proposing a TNL or writing code. Before locking interpretation, the agent MUST also surface — across the dimensions every system has — what it accepts and rejects (inputs), what it produces in success and error cases (outputs), what it interfaces with (boundaries — other code, services, processes, filesystem, network), and what signals confirm correct operation (observability). Categories with no relevant clauses MAY be left implicit, but the agent MUST be able to state why if asked. The agent MUST NOT lock in a single interpretation without surfacing it for review first.
  ...
  - A TNL MUST govern one coherent behavioral surface. "Coherent" means: the paths form a bounded set, the MUSTs share a single intent that fits in the \`intent:\` paragraph without subdivision, and the clauses can be reviewed and approved as a single decision. When a request spans multiple coherent surfaces, the agent MUST propose multiple TNLs — one per surface — not a single TNL covering all of them. Delivery boundaries (phase, milestone, sprint, release) are NOT TNL boundaries; contract scope and delivery scope are independent.
...
rationale:
  Each behavior targets a specific failure pattern observed in prior sessions:
    - Locking in an interpretation without surfacing it for review ships the wrong feature fast.
    ...
    - Describing what the system *is* without naming what it has to *handle* (inputs, outputs, boundaries, observability) lets implementation details be silently chosen at code time; forcing the author across these dimensions catches the gaps before approval.
    - Conflating delivery scope (phase, sprint, milestone) with contract scope (one coherent surface) produces fat TNLs that aggregate unrelated MUSTs, defeating reviewability and per-feature evolution.
`;
